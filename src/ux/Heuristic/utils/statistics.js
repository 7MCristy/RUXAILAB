 ...
 *
 * Parámetro:
 * @param {Array<{result, SumOfValues, totalNoAplication, totalWarnings, timeSpentMs}>} array
 ...
 * - maxPerfectResult = (qtdQuestion - qtdNoAplication) * maxOption
 * - maxPerfResultWarnings = (qtdQuestion - qtdNoAplication + totalWarnings) * maxOption
 * - result(%) = (result * 100) / maxPerfectResult
 * - baseWarning(%) = (result * 100) / maxPerfResultWarnings
 * - maxWarning(%) = ((result + totalWarnings * maxOption) * 100) / maxPerfResultWarnings
 * - minWarning(%) = ((result + totalWarnings * minOption) * 100) / maxPerfResultWarnings
 *
 * Retorna:
 * @returns {{
 *   result: string,
...
function calcFinalResult(array) {
  ...
  const minOption = optionValues.length ? Math.min(...optionValues) : 0
  ...
  array.forEach((res) => {
    ...
    heuristicsDetail.push({
      heuristicId: res.id,
      individualResult,
      SumOfValues: res.SumOfValues,
      totalNoAplication: res.totalNoAplication,
      totalWarnings: res.totalWarnings || 0,
      applicableForThisHeuristic: res.SumOfValues - res.totalNoAplication,
      timeSpentMs: res.timeSpentMs,
    })

    result += individualResult
    qtdQuestion += res.SumOfValues
    qtdNoAplication += res.totalNoAplication
    applicableQuestions += res.SumOfValues - res.totalNoAplication
    totalWarnings += res.totalWarnings || 0
  })

  const maxPerfectResult = applicableQuestions * maxOption
  const maxPerfResultWarnings =
    (applicableQuestions + totalWarnings) * maxOption
  const finalPct =
    maxPerfectResult === 0
      ? '0.00'
      : ((result * 100) / maxPerfectResult).toFixed(2)
  const baseWarning =
    maxPerfResultWarnings === 0
      ? '0.00'
      : ((result * 100) / maxPerfResultWarnings).toFixed(2)
  const maxWarning =
    maxPerfResultWarnings === 0
      ? '0.00'
      : (
      ...
      evaluator.heuristicQuestions.forEach((heuristic) => {
        ...
        let res = heuristic.heuristicQuestions.reduce(
          (SumOfValues, question) => {
            ...
            console.log(
              '[DEBUG] heuristicAnswer completo:',
              JSON.stringify(question.heuristicAnswer),
            )
            console.log(
              '[DEBUG] warning field:',
              question.heuristicAnswer?.warning,
            )
            console.log(
              '[DEBUG] typeof warning:',
              typeof question.heuristicAnswer?.warning,
            )
            if (
              question.heuristicAnswer?.value === null &&
            ...
            if (question.heuristicAnswer?.warning === true) {
              totalWarnings++
              console.log('[statistics] WARNING DETECTED in question:', {
                totalWarnings,
              })
            }
            const images = Array.isArray(question?.images)
              ? question.images
              : Array.isArray(question.heuristicAnswer?.images)
        ...
        SelectEvaluator.heuristics.push({
          id: `H${heurisIndex}`,
          result: res == -1 ? 0 : res,
          SumOfValues: heuristic.heuristicTotal,
 ...
 *        [{ userDocId, heuristics: [{id, result, SumOfValues, ...}], ... }, ...]
 ...
 *          max: 4 * 10,      // Puntuación máxima teórica (maxValue * SumOfValues)
 *          min: 0 * 10,      // Puntuación mínima teórica (minValue * SumOfValues)
      ...
      for (const heuristic of evaluator.heuristics) {
        ...
        if (item) {
          ...
          table.items.push({
            ...
            max: max * (heuristic.SumOfValues || 0),
            min: min * (heuristic.SumOfValues || 0),
